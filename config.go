...
const (
	defaultConfigFilename        = "flokicoind.conf"
	...
	sampleConfigFilename         = "sample-flokicoind.conf"
...
var (
	defaultHomeDir     = chainutil.AppDataDir("flokicoind", false)
...
// config defines the configuration options for flokicoind.
//
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause flokicoind to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause flokicoind to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the whitelist, and an empty whitelist will allow all agents that do not fail the blacklist."`
	...
	Generate             bool          `long:"generate" description:"Generate (mine) flokicoins using the CPU"`
	FreeTxRelayLimit     float64       `long:"limitfreerelay" description:"Limit relay of transactions with no transaction fee to the given amount in thousands of bytes per minute"`
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 15212, testnet: 25212)"`
	LogDir               string        `long:"logdir" description:"Directory to log output."`
	MaxOrphanTxs         int           `long:"maxorphantx" description:"Max number of orphan transactions to keep in memory"`
	MaxPeers             int           `long:"maxpeers" description:"Max number of inbound and outbound peers"`
	MiningAddrs          []string      `long:"miningaddr" description:"Add the specified payment address to the list of addresses to use for generated blocks -- At least one address is required if the generate option is set"`
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in FLC/kB to be considered a non-zero fee."`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 15216, testnet: 25216)"`
	RPCMaxClients        int           `long:"rpcmaxclients" description:"Max number of RPC clients for standard connections"`
	RPCMaxConcurrentReqs int           `long:"rpcmaxconcurrentreqs" description:"Max number of concurrent RPC requests that may be processed concurrently"`
	RPCMaxWebsockets     int           `long:"rpcmaxwebsockets" description:"Max number of RPC websocket connections"`
	RPCQuirks            bool          `long:"rpcquirks" description:"Mirror some JSON-RPC quirks of Flokicoin -- NOTE: Discouraged unless interoperability issues need to be worked around"`
...
//
// The above results in flokicoind functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:        mempool.DefaultMinRelayTxFee.ToFLC(),
	...
	if cfg.DisableRPC {
		flcdLog.Infof("RPC service is disabled")
	}
	...
	cfg.miningAddrs = make([]chainutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := chainutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if !cfg.DisableRPC && cfg.DisableTLS {
		...
		allowedTLSListeners["0.0.0.0"] = struct{}{}
		allowedTLSListeners["::"] = struct{}{}
	...
	if configFileError != nil {
		flcdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-flokicoind.conf to the given destination path,
