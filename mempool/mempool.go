...
type Config struct {
	...
	FetchUtxoView func(*chainutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*chainutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in FLC/kB to be
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*chainutil.Tx
	outpoints     map[wire.OutPoint]*chainutil.Tx
...
func (mp *TxPool) removeOrphan(tx *chainutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *chainutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *chainutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*chainutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *chainutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *chainutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *chainutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *chainutil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *chainutil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *chainutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*chainutil.Tx) map[chainhash.Hash]*chainutil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*chainutil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*chainutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *chainutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*chainutil.Tx) map[chainhash.Hash]*chainutil.Tx {
	...
	descendants := make(map[chainhash.Hash]*chainutil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *chainutil.Tx) map[chainhash.Hash]*chainutil.Tx {
	conflicts := make(map[chainhash.Hash]*chainutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *chainutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *chainutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*chainutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *chainutil.Tx,
	txFee int64) (map[chainhash.Hash]*chainutil.Tx, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *chainutil.Tx, isNew, rateLimit,
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *chainutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *chainutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*chainutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *chainutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
func (mp *TxPool) RawMempoolVerbose() map[string]*chainjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &chainjson.GetRawMempoolVerboseResult{
			...
			Fee:              chainutil.Amount(desc.Fee).ToFLC(),
...
type MempoolAcceptResult struct {
	// TxFee is the fees paid in loki.
	...
	// transaction(RBF).
	Conflicts map[chainhash.Hash]*chainutil.Tx
...
func (mp *TxPool) CheckMempoolAcceptance(tx *chainutil.Tx) (
...
func (mp *TxPool) checkMempoolAcceptance(tx *chainutil.Tx,
	...
	var conflicts map[chainhash.Hash]*chainutil.Tx
	if isReplacement {
...
func (mp *TxPool) validateSegWitDeployment(tx *chainutil.Tx) error {
...
// and input standard.
func (mp *TxPool) validateStandardness(tx *chainutil.Tx, nextBlockHeight int32,
...
func (mp *TxPool) validateSigCost(tx *chainutil.Tx,
...
// transaction.
func (mp *TxPool) validateRelayFeeMet(tx *chainutil.Tx, txFee, txSize int64,
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*chainutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*chainutil.Tx),
	}
}
