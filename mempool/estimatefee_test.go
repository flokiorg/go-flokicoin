...
func expectedFeePerKilobyte(t *TxDesc) FlcPerKilobyte {
	size := float64(t.TxDesc.Tx.MsgTx().SerializeSize())
	fee := float64(t.TxDesc.Fee)

	return LokiPerByte(fee / size).ToFlcPerKb()
}
...
func (eft *estimateFeeTester) estimates() [estimateFeeDepth]FlcPerKilobyte {

	// Generate estimates
	var estimates [estimateFeeDepth]FlcPerKilobyte
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	estimateHistory [][estimateFeeDepth]FlcPerKilobyte,
	txPerRound, txPerBlock uint32) ([][]*TxDesc, [][estimateFeeDepth]FlcPerKilobyte) {
...
func TestEstimateFeeRollback(t *testing.T) {
	...
	estimateHistory := [][estimateFeeDepth]FlcPerKilobyte{eft.estimates()}
...
func (eft *estimateFeeTester) checkSaveAndRestore(
	previousEstimates [estimateFeeDepth]FlcPerKilobyte) {
