...
func (m *MockTxMempool) RawMempoolVerbose() map[string]*chainjson.
	GetRawMempoolVerboseResult {

	args := m.Called()
	return args.Get(0).(map[string]*chainjson.GetRawMempoolVerboseResult)
}
...
func (m *MockTxMempool) FetchTransaction(
	txHash *chainhash.Hash) (*chainutil.Tx, error) {
	...
	return args.Get(0).(*chainutil.Tx), args.Error(1)
}
...
func (m *MockTxMempool) ProcessTransaction(tx *chainutil.Tx, allowOrphan,
...
func (m *MockTxMempool) CheckMempoolAcceptance(
	tx *chainutil.Tx) (*MempoolAcceptResult, error) {
...
func (m *MockTxMempool) CheckSpend(op wire.OutPoint) *chainutil.Tx {
	args := m.Called(op)
	...
	return args.Get(0).(*chainutil.Tx)
}
