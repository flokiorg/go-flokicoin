...
const (
	...
	// DefaultMinRelayTxFee is the minimum fee in loki that is required
	// for a transaction to be treated as free for relay and mining
	// purposes.  It is also used to help determine if a transaction is
	// considered dust and as a base for calculating minimum required fees
	// for larger transactions.  This value is in Loki/1000 bytes.
	DefaultMinRelayTxFee = chainutil.Amount(1000)
...
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee chainutil.Amount) int64 {
	...
	// free transaction relay fee).  minRelayTxFee is in Loki/kB so
	...
	if minFee < 0 || minFee > chainutil.MaxLoki {
		minFee = chainutil.MaxLoki
	}
...
func checkInputsStandard(tx *chainutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee chainutil.Amount) bool {
	...
	// fee of 1000, this equates to values less than 546 loki being
	// considered dust.
	//
...
func CheckTransactionStandard(tx *chainutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee chainutil.Amount,
...
func GetTxVirtualSize(tx *chainutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
