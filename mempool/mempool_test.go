...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee chainutil.Amount,
	signalsReplacement bool) (*chainutil.Tx, error) {
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := crypto.PrivKeyFromBytes(keyBytes)
	...
	harness := poolHarness{
		...
		txPool: New(&Config{
			Policy: Policy{
				...
				MinRelayTxFee:        1000, // 1 Loki per byte
				MaxTxVersion:         1,
			},
			ChainParams:      chainParams,
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *chainutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee chainutil.Amount,
	signalsReplacement, confirmed bool) *chainutil.Tx {
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *chainutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*chainutil.Tx, []*chainutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*chainutil.Tx, []*chainutil.Tx) {
				...
				return tx, []*chainutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*chainutil.Tx, []*chainutil.Tx) {
				...
				return tx, []*chainutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = chainutil.LokiPerFlokicoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*chainutil.Tx, []*chainutil.Tx) {
				...
				return childReplace, []*chainutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
