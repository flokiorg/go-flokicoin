...
type LokiPerByte float64

// FlcPerKilobyte is number with units of flokicoins per kilobyte.
type FlcPerKilobyte float64

// ToFlcPerKb returns a float value that represents the given
// LokiPerByte converted to lokis per kb.
func (rate LokiPerByte) ToFlcPerKb() FlcPerKilobyte {
	// If our rate is the error value, return that.
	if rate == LokiPerByte(-1.0) {
		return -1.0
	}

	return FlcPerKilobyte(float64(rate) * bytePerKb * flcPerLoki)
}

// Fee returns the fee for a transaction of a given size for
// the given fee rate.
func (rate LokiPerByte) Fee(size uint32) chainutil.Amount {
	// If our rate is the error value, return that.
	if rate == LokiPerByte(-1) {
...
// NewLokiPerByte creates a LokiPerByte from an Amount and a
// size in bytes.
func NewLokiPerByte(fee chainutil.Amount, size uint32) LokiPerByte {
	return LokiPerByte(float64(fee) / float64(size))
}
...
type observedTransaction struct {
	...
	// The fee per byte of the transaction in lokis.
	feeRate LokiPerByte
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewLokiPerByte(chainutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *chainutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*chainutil.Tx]struct{})
	...
	for t := range transactions {
		...
		// Refer to github.com/flokiorg/go-flokicoin/issues/1660 for more information.
		//
...
func (b *estimateFeeSet) estimateFee(confirmations int) LokiPerByte {
	if confirmations <= 0 {
		return LokiPerByte(math.Inf(1))
	}
...
func (ef *FeeEstimator) newEstimateFeeSet() *estimateFeeSet {
	...
	set.feeRate = make([]LokiPerByte, capacity)
...
func (ef *FeeEstimator) estimates() []LokiPerByte {
	set := ef.newEstimateFeeSet()

	estimates := make([]LokiPerByte, estimateFeeDepth)
...
func (ef *FeeEstimator) EstimateFee(numBlocks uint32) (FlcPerKilobyte, error) {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	return ef.cached[int(numBlocks)-1].ToFlcPerKb(), nil
}
