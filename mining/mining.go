...
type TxDesc struct {
	...
	// FeePerKB is the fee the transaction pays in Loki per 1000 bytes.
	FeePerKB int64
}
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress chainutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*chainutil.Tx, 0, len(sourceTxns))
...
func AddWitnessCommitment(coinbaseTx *chainutil.Tx,
	blockTxns []*chainutil.Tx) []byte {
