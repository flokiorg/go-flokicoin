	...
	requests := []chainjson.ImportMultiRequest{
		{
			...
			Range:     &chainjson.DescriptorRange{Value: []int{0, 100}},
			Timestamp: chainjson.TimestampOrNow{Value: 0}, // scan from genesis
			WatchOnly: chainjson.Bool(true),
			KeyPool:   chainjson.Bool(false),
			Internal:  chainjson.Bool(false),
		},
	}
	opts := &chainjson.ImportMultiOptions{Rescan: true}
...
func ExampleClient_GetTxOutSetInfo() {
	...
	fmt.Println(r.TotalAmount.String()) // 20947654.56996054 FLC
