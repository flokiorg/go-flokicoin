...
detected by type asserting the error in a *chainjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

	  amount, err := client.GetBalance("")
	  if err != nil {
	  	if jerr, ok := err.(*chainjson.RPCError); ok {
	  		switch jerr.Code {
	  		case chainjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
  ...
  - lokidwebsockets
    ...
    block count
  - walletdwebsockets
    Connects to a walletd RPC server using TLS-secured websockets, registers
