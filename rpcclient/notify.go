...
type notificationState struct {
	...
	notifySpent        map[chainjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[chainjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[chainjson.OutPoint]struct{}),
	}
}
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case chainjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case chainjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case chainjson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case chainjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case chainjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case chainjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case chainjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseChainTxNtfnParams(params []json.RawMessage) (*chainutil.Tx,
	*chainjson.BlockDetails, error) {
	...
	// JSON object.
	var block *chainjson.BlockDetails
	if len(params) > 1 {
...
func (c *Client) notifySpentInternal(outpoints []chainjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := chainjson.NewNotifySpentCmd(outpoints)
	return c.SendCmd(cmd)
}

// newOutPointFromWire constructs the chainjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) chainjson.OutPoint {
	return chainjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]chainjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := chainjson.NewNotifySpentCmd(ops)
	return c.SendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := chainjson.NewNotifyReceivedCmd(addresses)
	return c.SendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(addresses []chainutil.Address) FutureNotifyReceivedResult {
	...
	cmd := chainjson.NewNotifyReceivedCmd(addrs)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := chainjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := chainjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.SendCmd(cmd)
}
