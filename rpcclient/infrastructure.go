...
// Client represents a Flokicoin RPC client which allows easy access to the
// various RPC methods available on a Flokicoin RPC server.  Each of the wrapper
...
// returning an unmarshaled chainjson.RPCError (or an unmarshalling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]chainjson.OutPoint, 0, nslen)
...
func (c *Client) SendCmd(cmd interface{}) chan *Response {
	rpcVersion := chainjson.RpcVersion1
	if c.batch {
		rpcVersion = chainjson.RpcVersion2
	}
	// Get the method associated with the command.
	method, err := chainjson.CmdMethod(cmd)
	if err != nil {
		return newFutureError(err)
	}
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := chainjson.MarshalCmd(rpcVersion, id, cmd)
	if err != nil {
		return newFutureError(err)
	}
