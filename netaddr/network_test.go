...
func TestIPTypes(t *testing.T) {
	...
	newIPTest := func(ip string, rfc1918, rfc2544, rfc3849, rfc3927, rfc3964,
		...
		nip := net.ParseIP(ip)
		na := *wire.NewNetAddressIPPort(nip, 15212, wire.SFNodeNetwork)
	...
	for _, test := range tests {
		...
		if rv := netaddr.IsRoutable(currentNa); rv != test.routable {
...
func TestGroupKey(t *testing.T) {
	...
	for i, test := range tests {
		...
		na := wire.NetAddressV2FromBytes(
			time.Now(), wire.SFNodeNetwork, nip, 15212,
		)
		if key := netaddr.GroupKey(na); key != test.expected {
