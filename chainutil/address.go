...
// and netID which encodes the flokicoin network and address type.  It is used
...
//
// The flokicoin network the address is associated with is extracted if possible.
...
// with the passed flokicoin network.
...
func NewAddressPubKey(serializedPubKey []byte, net *chaincfg.Params) (*AddressPubKey, error) {
	pubKey, err := crypto.ParsePubKey(serializedPubKey)
	if err != nil {
		return nil, err
	}

	// Set the format of the pubkey.  This probably should be returned
	// from crypto package, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Flokicoin addresses
