...
import (
	...
	"github.com/flokiorg/go-flokicoin/chainutil/base58"
...
var masterKey = []byte("Flokicoin seed")
...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		_, pubKey := crypto.PrivKeyFromBytes(k.key)
...
func (k *ExtendedKey) Derive(i uint32) (*ExtendedKey, error) {
	...
	if k.isPrivate {
		...
		var (
			ilScalar crypto.ModNScalar
			ilJ      crypto.JacobianPoint
		)
		if overflow := ilScalar.SetByteSlice(il); overflow {
			return nil, ErrInvalidChild
		}
		crypto.ScalarBaseMultNonConst(&ilScalar, &ilJ)
		...
		// public key.
		pubKey, err := crypto.ParsePubKey(k.key)
		if err != nil {
			return nil, err
		}
		...
		var pubKeyJ crypto.JacobianPoint
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		var childKeyPubJ crypto.JacobianPoint
		crypto.AddNonConst(&ilJ, &pubKeyJ, &childKeyPubJ)
		...
		childKeyPub := crypto.NewPublicKey(
	...
	parentFP := chainutil.Hash160(k.pubKeyBytes())[:4]
...
func (k *ExtendedKey) DeriveNonStandard(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(crypto.S256().N) >= 0 || ilNum.Sign() == 0 {
		return nil, ErrInvalidChild
	}
...
func (k *ExtendedKey) CloneWithVersion(version []byte) (*ExtendedKey, error) {
	if len(version) != 4 {
		...
		// ErrInvalidHDKeyID. Update the  error type once available in a stable
		// release.
...
// ECPubKey converts the extended key to a crypto public key and returns it.
func (k *ExtendedKey) ECPubKey() (*crypto.PublicKey, error) {
	return crypto.ParsePubKey(k.pubKeyBytes())
}

// ECPrivKey converts the extended key to a crypto private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*crypto.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := crypto.PrivKeyFromBytes(k.key)
	return privKey, nil
}

// Address converts the extended key to a standard flokicoin pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*chainutil.AddressPubKeyHash, error) {
	pkHash := chainutil.Hash160(k.pubKeyBytes())
	return chainutil.NewAddressPubKeyHash(pkHash, net)
}
...
func NewCustomMaster(mKey, seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	//   I = HMAC-SHA512(Key = "Flokicoin seed", Data = S)
	hmac512 := hmac.New(sha512.New, mKey)
	...
	if secretKeyNum.Cmp(crypto.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	return NewCustomMaster(masterKey, seed, net)
}
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(crypto.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := crypto.ParsePubKey(keyData)
		if err != nil {
			return nil, err
		}
	}
