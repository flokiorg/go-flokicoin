...
A Block defines a flokicoin block that provides easier and more efficient
...
A Tx defines a flokicoin transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Flokicoin address.  While the
most common type is a pay-to-pubkey-hash, Flokicoin already supports others and
...
To decode/encode an address:
	...
	addr, err := chainutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
