...
func TestNewTxFromBytes(t *testing.T) {
	...
	tx, err := chainutil.NewTxFromBytes(testTxBytes)
	if err != nil {
...
func TestTxErrors(t *testing.T) {
	...
	_, err = chainutil.NewTxFromBytes(shortBytes)
	if err != io.EOF {
...
func TestTxHasWitness(t *testing.T) {
	...
	for _, msgTxWithoutWitness := range Block100000.Transactions[1:] {
		txWithoutWitness := chainutil.NewTx(msgTxWithoutWitness)
