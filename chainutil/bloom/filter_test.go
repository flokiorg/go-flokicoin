...
func TestFilterInsert(t *testing.T) {
	...
	err = f.MsgFilterLoad().FlcEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsert FlcDecode failed: %v\n", err)
		return
	}
...
func TestFilterFPRange(t *testing.T) {
	...
	for _, test := range tests {
		...
		err = f.MsgFilterLoad().FlcEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
		if err != nil {
			t.Errorf("FlcDecode unexpected error: %v\n", err)
			continue
		}
		if !bytes.Equal(got.Bytes(), want) {
...
func TestFilterInsertWithTweak(t *testing.T) {
	...
	err = f.MsgFilterLoad().FlcEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsertWithTweak FlcDecode failed: %v\n", err)
		return
	}
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := chainutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(chainutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := chainutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := chainutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
