...
type WIF struct {
	...
	// netID is the flokicoin network identifier byte used when
...
func NewWIF(privKey *crypto.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + crypto.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + crypto.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+crypto.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+crypto.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+crypto.PrivKeyBytesLen]
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + crypto.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
