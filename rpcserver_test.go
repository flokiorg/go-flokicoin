    ...
    cases := []struct {
        ...
        },
        {
            ...
            setup: func(t *testing.T) (*rpcServer, *chainjson.SubmitAuxBlockCmd) {
                ...
                t.Cleanup(cleanup)
                s := &rpcServer{cfg: rpcserverConfig{Chain: chain, ChainParams: &params, AuxCache: newAuxCache(2 * time.Minute)}}
                cand := &AuxCandidate{Hash: chainhash.DoubleHashH([]byte("X")), PrevHash: *params.GenesisHash, Height: 1, Bits: params.PowLimitBits, Target: blockchain.CompactToBig(params.PowLimitBits), CoinbaseValue: 0, Template: &mining.BlockTemplate{Block: &wire.MsgBlock{Header: wire.BlockHeader{PrevBlock: *params.GenesisHash, Bits: params.PowLimitBits}}}}
        ...
        },
        {
            ...
            setup: func(t *testing.T) (*rpcServer, *chainjson.SubmitAuxBlockCmd) {
                ...
                t.Cleanup(cleanup)
                s := &rpcServer{cfg: rpcserverConfig{Chain: chain, ChainParams: &params, AuxCache: newAuxCache(2 * time.Minute)}}
                cand := &AuxCandidate{Hash: chainhash.DoubleHashH([]byte("Y")), PrevHash: *params.GenesisHash, Height: 1, Bits: params.PowLimitBits, Target: blockchain.CompactToBig(params.PowLimitBits), CoinbaseValue: 0, Template: &mining.BlockTemplate{Block: &wire.MsgBlock{Header: wire.BlockHeader{PrevBlock: *params.GenesisHash, Bits: params.PowLimitBits}}}}
                s.cfg.AuxCache.put(cand)
