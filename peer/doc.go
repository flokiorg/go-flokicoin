...
/*
Package peer provides a common base for creating and managing Flokicoin network
peers.

# Overview

This package builds upon the wire package, which provides the fundamental
primitives necessary to speak the flokicoin wire protocol, in order to simplify
...
A quick overview of the major features peer provides are as follows:
  ...
  - Provides a basic concurrent safe flokicoin peer for handling flokicoin
    communications via the peer-to-peer protocol
  - Full duplex reading and writing of flokicoin protocol messages
  ...
  - Flexible peer configuration
    ...
    6. Ability to register callbacks for handling flokicoin protocol messages
...
caller to specify things such as the user agent name and version, the flokicoin
network to use, which services it supports, and callbacks to invoke when flokicoin
...
In order to do anything useful with a peer, it is necessary to react to flokicoin
...
For convenience, a callback hook for all of the currently supported flokicoin
...
function which allows a flog.Logger to be specified.  For example, logging at
...
(https://pkg.go.dev/github.com/flokiorg/go-flokicoin/wire#hdr-Bitcoin_Improvement_Proposals)
*/
package peer
