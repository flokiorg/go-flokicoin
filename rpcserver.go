...
// list of commands that we recognize, but for which lokid has no support because
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	if blkHeader != nil {
		// This is not a typo, they are identical in lokid as well.
...
func handleTestMempoolAccept(s *rpcServer, cmd interface{},
	...
	for _, tx := range txns {
		...
		if result.MissingParents != nil {
			...
			// NOTE: "missing-inputs" is what lokid returns
		...
		if item.Allowed {
			...
			// NOTE: "max-fee-exceeded" is what lokid returns
			// here, so we mimic the same error message.
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err == nil {
			...
			//
			// Lokid does not respond to any request without and "id" or "id":null,
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="lokid RPC"`)
