...
var (
	...
	// data.
	gbtCoinbaseAux = &chainjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &chainjson.RPCError{
	...
	ErrRPCNoWallet = &chainjson.RPCError{
		Code:    chainjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which flokicoind has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of walletd.
...
// not needed.
func internalRPCError(errStr, context string) *chainjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return chainjson.NewRPCError(chainjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *chainjson.RPCError {
	return chainjson.NewRPCError(chainjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *chainjson.RPCError {
	return chainjson.NewRPCError(chainjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in walletd.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.FlcEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		addr, err := chainutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &chainjson.RPCError{
...
// transaction.
func createVinList(mtx *wire.MsgTx) []chainjson.Vin {
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []chainjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = chainutil.Amount(v.Value).ToFLC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*chainjson.TxRawResult, error) {
	...
	txReply := &chainjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(chainutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(chainutil.NewTx(mtx))),
	...
	if blkHeader != nil {
		// This is not a typo, they are identical in flokicoind as well.
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &chainjson.RPCError{
			Code:    chainjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := chainjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.DecodeScriptCmd)
	...
	p2sh, err := chainutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := chainjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	return handleEstimateSmartFee(s, cmd, closeChan)
}

// handleEstimateSmartFee handles estimatefee commands.
func handleEstimateSmartFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.EstimateSmartFeeCmd)
	...
	// Convert to lokis per kb.
	feerate, err := chainutil.NewAmount(float64(feeRate))
	if err != nil {
		return -1.0, &chainjson.RPCError{
			Code:    chainjson.ErrRPCInternal.Code,
			Message: "parameter NumBlocks must be positive",
	...
	rate := feerate.ToUnit(chainutil.AmountFLC)
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !s.cfg.ChainParams.GenerateSupported {
		return nil, &chainjson.RPCError{
			Code: chainjson.ErrRPCDifficulty,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &chainjson.RPCError{
				Code:    chainjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*chainjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result chainjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = chainjson.Bool(peer.Connected())
		...
		addrs := make([]chainjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr chainjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &chainjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := chainutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	chainWork, err := s.cfg.Chain.ChainWork(hash)
	if err != nil {
		return nil, internalRPCError(err.Error(), "Could not fetch chain work")
	}
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &chainjson.GetBlockChainInfoResult{
		...
		Pruned:        cfg.Prune != 0,
		SoftForks: &chainjson.SoftForks{
			Bip9SoftForks: make(map[string]*chainjson.Bip9SoftForkDescription),
		},
		VerificationProgress: verifyProgress,
		InitialBlockDownload: ibdFlag,
		ChainWork:            fmt.Sprintf("%064x", chainWork),
		SizeOnDisk:           100,
		Warnings: []string{
	...
	chainInfo.SoftForks.SoftForks = []*chainjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &chainjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetBlockHashCmd)
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := chainjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	if template == nil || state.prevHash == nil ||
		...
		msgBlock = template.Block
		targetDifficulty = fmt.Sprintf("%064x", blockchain.CompactToBig(msgBlock.Header.Bits))
...
// state as a chainjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*chainjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]chainjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := chainutil.NewTx(tx)
		resultTx := chainjson.GetBlockTemplateResultTx{
	...
	reply := chainjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *chainjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &chainjson.RPCError{
			Code:    chainjson.ErrRPCClientNotConnected,
			Message: "Flokicoin is not connected",
		}
	}
	...
	// 	return nil, &chainjson.RPCError{
	// 		Code:    chainjson.ErrRPCClientInInitialDownload,
	// 		Message: "Flokicoin is downloading blocks...",
	// 	}
	// }
...
func handleGetBlockTemplateProposal(s *rpcServer, request *chainjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &chainjson.RPCError{
			Code: chainjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetChainTips(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := make([]chainjson.GetChainTipsResult, 0, len(chainTips))
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &chainjson.RPCError{
			Code:    chainjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*chainjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*chainjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetIndexInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	best := s.cfg.Chain.BestSnapshot()
	ret := &chainjson.IndexInfoResult{
		TxIndex: chainjson.TxIndexResult{
			Synced:          true,
...
func handleGetBlockStats(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetBlockStatsCmd)

	var blockHash *chainhash.Hash
	var err error

	// Determine if HashOrHeight is a block hash or height
	if height, ok := c.HashOrHeight.Value.(int32); ok {
		blockHash, err = s.cfg.Chain.BlockHashByHeight(height)
		if err != nil {
			return nil, err
		}
	} else if strHeight, ok := c.HashOrHeight.Value.(string); ok {
		blockHash, err = chainhash.NewHashFromStr(strHeight)
		if err != nil {
			return nil, err
		}
	} else {
		return nil, fmt.Errorf("unknown height format")
	}
	...
	// Fetch block header and its height
	header, err := s.cfg.Chain.HeaderByHash(blockHash)
	if err != nil {
		return nil, err
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &chainjson.GetMempoolInfoResult{
		Size:             int64(len(mempoolTxns)),
		Bytes:            numBytes,
		Loaded:           true,
		Usage:            int64(numBytes * 2), // static memory usage
		MaxMempool:       300_000_000,
		MempoolMinFee:    0.0, // 0.00001988
		MinRelayTxFee:    0.0, // 0.00001000
		UnbroadcastCount: 0,
	}
...
func handleGetMempoolEntry(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetMempoolEntryCmd)
	...
	for _, desc := range s.cfg.TxMemPool.TxDescs() {
		if !desc.Tx.Hash().IsEqual(txHash) {
			continue
		}
		return &chainjson.GetMempoolEntryResult{
			VSize:           int32(mempool.GetTxVirtualSize(desc.Tx)),
			Weight:          blockchain.GetTransactionWeight(desc.Tx),
			Fee:             chainutil.Amount(desc.Fee).ToFLC(),
			ModifiedFee:     chainutil.Amount(desc.Fee).ToFLC(),
			Time:            desc.Added.Unix(),
			Height:          int64(desc.Height),
			DescendantCount: 1,
			DescendantSize:  mempool.GetTxVirtualSize(desc.Tx),
			DescendantFees:  chainutil.Amount(desc.Fee).ToFLC(),
			AncestorCount:   1,
			AncestorSize:    mempool.GetTxVirtualSize(desc.Tx),
			AncestorFees:    chainutil.Amount(desc.Fee).ToFLC(),
			WTxId:           desc.Tx.WitnessHash().String(),
			Fees: chainjson.MempoolFees{
				Base:       chainutil.Amount(desc.Fee).ToFLC(),
				Modified:   chainutil.Amount(desc.Fee).ToFLC(),
				Ancestor:   chainutil.Amount(desc.Fee).ToFLC(),
				Descendant: chainutil.Amount(desc.Fee).ToFLC(),
			},
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := chainjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := chainjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &chainjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*chainjson.GetNetworkHashPSCmd)
...
func handleGetNodeAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetNodeAddressesCmd)
	...
	addresses := make([]*chainjson.GetNodeAddressesResult, 0, count)
	for _, node := range nodes[:count] {
		address := &chainjson.GetNodeAddressesResult{
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*chainjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &chainjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &chainjson.RPCError{
				Code: chainjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &chainjson.GetTxOutResult{
		...
		Value:         chainutil.Amount(value).ToFLC(),
...
func handleInvalidateBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.InvalidateBlockCmd)
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]chainjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]chainjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]chainjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &chainjson.PrevOut{
				...
				Value:     chainutil.Amount(originTxOut.Value).ToFLC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr chainutil.Address, numToSkip, numRequested uint32) ([]*chainutil.Tx, uint32) {
...
func handleReconsiderBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.ReconsiderBlockCmd)
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*chainjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := chainutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &chainjson.RPCError{
			Code:    chainjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]chainjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
		...
		if blkHeader != nil {
			// This is not a typo, they are identical in Flokicoin
			// Core as well.
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		// matching flokicoind's behavior.
		code := chainjson.ErrRPCTxError
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = chainjson.ErrRPCTxAlreadyInChain

			default:
				code = chainjson.ErrRPCTxRejected
			}
		}
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.SetGenerateCmd)
...
const messageSignatureHeader = "Flokicoin Signed Message:\n"
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.SignMessageWithPrivKeyCmd)

	wif, err := chainutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case chainutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case chainutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.ValidateAddressCmd)

	result := chainjson.ValidateAddressChainResult{}
	addr, err := chainutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
	...
	case *chainutil.AddressPubKeyHash:
		result.IsScript = chainjson.Bool(false)
		result.IsWitness = chainjson.Bool(false)

	case *chainutil.AddressScriptHash:
		result.IsScript = chainjson.Bool(true)
		result.IsWitness = chainjson.Bool(false)
	...
	case *chainutil.AddressWitnessPubKeyHash:
		result.IsScript = chainjson.Bool(false)
		result.IsWitness = chainjson.Bool(true)
		result.WitnessVersion = chainjson.Int32(int32(addr.WitnessVersion()))
		result.WitnessProgram = chainjson.String(hex.EncodeToString(addr.WitnessProgram()))
	...
	default:
		// Handle the case when a new Address is supported by chainutil, but none
	...
	result.IsValid = true
	result.ScriptPubKey = hex.EncodeToString(scriptPubkey)
...
func handleVerifyChain(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.VerifyChainCmd)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.VerifyMessageCmd)
	...
	pk, wasCompressed, err := ecdsa.RecoverCompact(sig,
		expectedMessageHash)
	if err != nil {
		// Mirror Flokicoin behavior, which treats error in
	...
	address, err := chainutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
		// Again mirror Flokicoin behavior, which treats error in public key
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]chainjson.VersionResult{
		"jsonrpcapi": {
...
func handleTestMempoolAccept(s *rpcServer, cmd interface{},
	...
	c := cmd.(*chainjson.TestMempoolAcceptCmd)

	// Create txns to hold the decoded tx.
	txns := make([]*chainutil.Tx, 0, len(c.RawTxns))
	...
	results := make([]*chainjson.TestMempoolAcceptResult, 0, len(txns))
	for _, tx := range txns {
		// Create a test result item.
		item := &chainjson.TestMempoolAcceptResult{
			Txid:  tx.Hash().String(),
		...
		if result.MissingParents != nil {
			...
			// NOTE: "missing-inputs" is what flokicoind returns
		...
		if item.Allowed {
			...
			// NOTE: "max-fee-exceeded" is what flokicoind returns
			// here, so we mimic the same error message.
...
func handleGetTxSpendingPrevOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*chainjson.GetTxSpendingPrevOutCmd)
	...
	results := make([]*chainjson.GetTxSpendingPrevOutResult, 0, len(ops))
	for _, op := range ops {
		// Create a result entry.
		result := &chainjson.GetTxSpendingPrevOutResult{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
...
func validateFeeRate(feeSats chainutil.Amount, txSize int64,
	maxFeeRate float64) (*chainjson.TestMempoolAcceptFees, bool) {

	// Calculate fee rate in sats/kvB.
	feeRateSatsPerKVB := feeSats * 1e3 / chainutil.Amount(txSize)

	// Convert sats/vB to FLC/kvB.
	feeRate := feeRateSatsPerKVB.ToFLC()

	// Get the max fee rate, if not provided, default to 0.1 FLC/kvB.
	if maxFeeRate == 0 {
	...
	return &chainjson.TestMempoolAcceptFees{
		Base:             feeSats.ToFLC(),
...
type parsedRPCCmd struct {
	jsonrpc chainjson.RPCVersion
	id      interface{}
	method  string
	cmd     interface{}
	err     *chainjson.RPCError
}

// standardCmdResult checks that a parsed command is a standard Flokicoin JSON-RPC
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, chainjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *chainjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(chainjson.Error); ok &&
			jerr.ErrorCode == chainjson.ErrUnregisteredMethod {

			parsedCmd.err = chainjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = chainjson.NewRPCError(
			chainjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *chainjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion chainjson.RPCVersion, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *chainjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*chainjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return chainjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *chainjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
	...
	if jsonErr == nil {
		...
		if parsedCmd.err != nil {
			...
			result, err = s.standardCmdResult(parsedCmd,
				closeChan)
			if err != nil {
				if rpcErr, ok := err.(*chainjson.RPCError); ok {
					jsonErr = rpcErr
				} else {
					jsonErr = &chainjson.RPCError{
						Code:    chainjson.ErrRPCInvalidRequest.Code,
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = chainjson.MarshalResponse(chainjson.RpcVersion1, nil, nil, jsonErr)
			if err != nil {
		...
		if err == nil {
			...
			//
			// Flokicoin serves requests with "id":null or even an absent "id",
			// and responds to such requests with "id":null in the response.
			//
			// Flokicoind does not respond to any request without and "id" or "id":null,
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = chainjson.MarshalResponse(chainjson.RpcVersion2, nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = chainjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="flokicoind RPC"`)
...
func genCertPair(certFile, keyFile string) error {
	...
	org := "flokicoind autogenerated cert"
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *chainutil.Block, flags blockchain.BehaviorFlags) (bool, error)
