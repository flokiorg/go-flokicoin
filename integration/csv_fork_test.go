...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt chainutil.Amount) (*crypto.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := chainutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	flokicoindCfg := []string{"--rejectnonstd"}
	...
	const outputValue = chainutil.LokiPerFlokicoin
	...
	txns := []*chainutil.Tx{chainutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numLokis chainutil.Amount, timeLock int32,
	...
	p2shAddr, err := chainutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = chainutil.LokiPerFlokicoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*chainutil.Tx{chainutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*chainutil.Tx{chainutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
