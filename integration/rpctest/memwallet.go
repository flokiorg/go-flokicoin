...
type memWallet struct {
	...
	coinbaseAddr chainutil.Address
	...
	addrs map[uint32]chainutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	// newly generated flokicoin we can spend.
	addrs := make(map[uint32]chainutil.Address)
...
// SetRPCClient saves the passed rpc connection to flokicoind as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*chainutil.Tx) {
...
func (m *memWallet) newAddress() (chainutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []chainutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
// fundTx attempts to fund a transaction sending amt flokicoin. The coins are
...
// lokis-per-byte. The transaction being funded can optionally include a
...
// expressed in lokis-per-byte. The transaction being created can optionally
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	for i, txIn := range tx.TxIn {
		...
		privKey, _ := crypto.PrivKeyFromBytes(privKeyOld.Serialize())
...
func keyToAddr(key *crypto.PrivateKey, net *chaincfg.Params) (chainutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := chainutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
