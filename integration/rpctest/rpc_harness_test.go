...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt chainutil.Amount) *chainhash.Hash {
		...
		// Next, send amt FLC to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(chainutil.Amount(5 * chainutil.LokiPerFlokicoin))
	...
	txid = genSpend(chainutil.Amount(500 * chainutil.LokiPerFlokicoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(chainutil.LokiPerFlokicoin, pkScript)

	const numTxns = 5
	txns := make([]*chainutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, chainutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 FLC.
	expectedBalance := chainutil.Amount(250 * chainutil.LokiPerFlokicoin)
	...
	// The original wallet should now have a balance of 0 FLC as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = chainutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := chainutil.Amount(50 * chainutil.LokiPerFlokicoin)
	...
	// The current wallet balance should now be at least 50 FLC less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 FLC) of mature unspendable
	// outputs.
	expectedBalance := chainutil.Amount(numMatureOutputs * 50 * chainutil.LokiPerFlokicoin)
