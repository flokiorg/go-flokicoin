...
import (
	...
	peerpkg "github.com/flokiorg/go-flokicoin/peer"
...
// blockMsg packages a flokicoin block message and the peer it came from together
...
// invMsg packages a flokicoin inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *peerpkg.Peer
...
// headersMsg packages a flokicoin headers message and the peer it came from
...
// notFoundMsg packages a flokicoin notfound message and the peer it came from
// together so the block handler has access to that information.
...
// txMsg packages a flokicoin tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx    *chainutil.Tx
	peer  *peerpkg.Peer
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// NOTE:  FlokicoinJ, and possibly other wallets, don't follow the spec of
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		...
		for _, tx := range block.Transactions()[1:] {
			sm.txMemPool.RemoveTransaction(tx, false, mempool.RemovalReasonBlock)
	...
	case blockchain.NTBlockDisconnected:
		...
		for _, tx := range block.Transactions()[1:] {
			...
			if err != nil {
				...
				// the transaction pool.
				sm.txMemPool.RemoveTransaction(tx, true, mempool.RemovalReasonReorg)
			}
		}
