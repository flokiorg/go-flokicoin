...
bitcoind standard.  There are a few key
differences between lokid and bitcoind as far as how RPCs are serviced:
...
The original bitcoind JSON-RPC API documentation is available at [https://en.bitcoin.it/wiki/Original_Bitcoin_client/API_Calls_list](https://en.bitcoin.it/wiki/Original_Bitcoin_client/API_Calls_list)
...
The following is an overview of the RPC methods which are implemented by lokid, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network lokid is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network lokid is running on.|
|Returns|numeric|
