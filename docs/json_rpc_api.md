...
flokicoind provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that follow the 
bitcoind standard.  There are a few key
differences between flokicoind and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, flokicoind intentionally splits the wallet and chain
  services into independent processes.  This means that if you are
  talking directly to flokicoind, only chain-related RPCs are available.
* flokicoind is secure by default which means that the RPC connection is TLS-enabled
  by default
* flokicoind provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for flokicoind RPC and are used by applications for inter-process
communication with flokicoind.  The websocket connection endpoint for flokicoind is
...
The flokicoind RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
flokicoind is `wss://your_ip_or_domain:15216/ws`.
...
to a flokicoind RPC server:

* **rpcuser** is the full-access username configured for the flokicoind RPC server
* **rpcpass** is the full-access password configured for the flokicoind RPC server
* **rpclimituser** is the limited username configured for the flokicoind RPC server
* **rpclimitpass** is the limited password configured for the flokicoind RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the flokicoind
  server is configured with.  It is automatically generated by flokicoind and placed
  in the flokicoind home directory (which is typically `%LOCALAPPDATA%\flokicoind` on
  Windows and `~/.flokicoind` on POSIX-like OSes)

**NOTE:** As mentioned above, flokicoind is secure by default which means the RPC
...
The flokicoind RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">flokicoind does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since flokicoind does not have the wallet integrated to provide payment addresses, flokicoind must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown flokicoind.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since flokicoind does not have a wallet integrated, flokicoind will only return whether the address is valid or not.|
...
|Notes|NOTE: Since flokicoind does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in walletd for a version which includes that information.|
...
|Notes|<font color="orange">Since flokicoind does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since flokicoind does not have the wallet integrated to provide payment addresses, flokicoind must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">flokicoind does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown flokicoind.|
|Returns|`"flokicoind stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For flokicoind this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Flokicoind currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for flokicoind.</font>|
...
The following is an overview of the RPC methods which are implemented by flokicoind, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network flokicoind is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network flokicoind is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of flokicoind.|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this flokicoind instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
flokicoind uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll flokicoind for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a flokicoind instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by flokicoind when it starts the RPC server and doesn't already
	// have one.
	homeDir := chainutil.AppDataDir("flokicoind", false)
...
The following is an example Go application which uses the
[rpcclient](https://github.com/flokiorg/go-flokicoin/tree/main/rpcclient) package to connect with
a flokicoind instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/flokiorg/go-flokicoin/tree/main/rpcclient) package to connect with
a flokicoind instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a flokicoind instance,
...
// generated by flokicoind when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/flokicoind/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The flokicoind generated certificate acts as
