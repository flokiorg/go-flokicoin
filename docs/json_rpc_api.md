...
|Notes|NOTE: Since flokicoind does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in btcwallet for a version which includes that information.|
...
|Notes|<font color="orange">Btcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for flokicoind.</font>|
...
[rpcclient](https://github.com/flokiorg/go-flokicoin/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/flokiorg/go-flokicoin/tree/master/rpcclient) package to connect with
...
func main() {
	...
	// have one.
	btcdHomeDir := chainutil.AppDataDir("flokicoind", false)
