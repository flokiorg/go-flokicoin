...
func ComputeTaprootOutputKey(pubKey *crypto.PublicKey,
	scriptRoot []byte) *crypto.PublicKey {
	...
	// N.
	var tweakScalar crypto.ModNScalar
	...
	var internalPoint crypto.JacobianPoint
	...
	var tPoint, taprootKey crypto.JacobianPoint
	crypto.ScalarBaseMultNonConst(&tweakScalar, &tPoint)
	crypto.AddNonConst(&internalPoint, &tPoint, &taprootKey)

	// Finally, we'll convert the key back to affine coordinates so we can
	// return the format of public key we usually use.
	taprootKey.ToAffine()

	return crypto.NewPublicKey(&taprootKey.X, &taprootKey.Y)
}
...
func ComputeTaprootKeyNoScript(internalKey *crypto.PublicKey) *crypto.PublicKey {
...
func TweakTaprootPrivKey(privKey crypto.PrivateKey,
	scriptRoot []byte) *crypto.PrivateKey {
	...
	return crypto.PrivKeyFromScalar(privTweak)
}
...
func (t *TapscriptProof) ToControlBlock(internalKey *crypto.PublicKey) ControlBlock {
...
func PayToTaprootScript(taprootKey *crypto.PublicKey) ([]byte, error) {
