...
func computeNonWitnessPkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) >= minPubKeyHashSigScriptLen &&
		...
		if crypto.IsCompressedPubKey(pubKey) {
...
func AddrToScripthash(addr chainutil.Address) ([]byte, error) {
	...
	scripthash := sha256.Sum256(addrScript)
	return reverseBytes(scripthash[:]), nil
}
