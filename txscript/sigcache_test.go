...
// test data.
func genRandomSig() (*chainhash.Hash, *ecdsa.Signature, *crypto.PublicKey, error) {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	key1Copy, _ := crypto.ParsePubKey(key1.SerializeCompressed())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		keyCopy, err := crypto.ParsePubKey(key.SerializeCompressed())
		if err != nil {
	...
	keyNewCopy, _ := crypto.ParsePubKey(keyNew.SerializeCompressed())
