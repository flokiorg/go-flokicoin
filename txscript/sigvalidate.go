...
// point in Flokicoin's history, there're four possible sig validation contexts:
...
func parseBaseSigAndPubkey(pkBytes, fullSigBytes []byte,
	vm *Engine) (*crypto.PublicKey, *ecdsa.Signature, SigHashType, error) {
	...
	// encoding.
	pubKey, err := crypto.ParsePubKey(pkBytes)
	if err != nil {
		return nil, nil, 0, err
	}
...
) (*crypto.PublicKey, *schnorr.Signature, SigHashType, error) {
