...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *crypto.PrivateKey) ([]byte, error) {
...
// WitnessSignature creates an input witness stack for tx to spend FLC sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *crypto.PrivateKey,
...
func TaprootWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *crypto.PrivateKey) (wire.TxWitness, error) {
...
func RawTxInTapscriptSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	privKey *crypto.PrivateKey) ([]byte, error) {
...
// SignatureScript creates an input signature script for tx to spend FLC sent
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *crypto.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *crypto.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []chainutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
// versions.
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []chainutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*chainutil.AddressPubKey)
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []chainutil.Address,
...
type KeyDB interface {
	GetKey(chainutil.Address) (*crypto.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(chainutil.Address) (*crypto.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address chainutil.Address) (*crypto.PrivateKey, bool, error) {
	return kc(address)
}
