...
// These constants represent the official opcode values used in the wiki,
// core implementations, and most other references and software related to handling scripts.
const (
	OP_0                   = 0x00 // 0
	...
	OP_SMALLINTEGER        = 0xfa // 250 - flokicoin internal
	OP_PUBKEYS             = 0xfb // 251 - flokicoin internal
	OP_UNKNOWN252          = 0xfc // 252
	OP_PUBKEYHASH          = 0xfd // 253 - flokicoin internal
	OP_PUBKEY              = 0xfe // 254 - flokicoin internal
	OP_INVALIDOPCODE       = 0xff // 255 - flokicoin internal
)
...
//
// Due to a bug in the original Loki client implementation, an additional
...
func opcodeCheckMultiSig(op *opcode, data []byte, vm *Engine) error {
	...
	// A bug in the original Loki client implementation means one more
	...
	for numSignatures > 0 {
		...
		// Parse the pubkey.
		parsedPubKey, err := crypto.ParsePubKey(pubKey)
		if err != nil {
			continue
		}
