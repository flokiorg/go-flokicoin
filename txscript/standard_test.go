...
// newAddressPubKey returns a new chainutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) chainutil.Address {
	addr, err := chainutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new chainutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) chainutil.Address {
...
// newAddressScriptHash returns a new chainutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) chainutil.Address {
	addr, err := chainutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// newAddressTaproot returns a new chainutil.AddressTaproot from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressTaproot(scriptHash []byte) chainutil.Address {
	addr, err := chainutil.NewAddressTaproot(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
		panic("invalid script hash in test source")
	}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			addrs: []chainutil.Address{newAddressTaproot(
...
// bogusAddress implements the chainutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the chainutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := chainutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := chainutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := chainutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := chainutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := chainutil.NewAddressPubKey(hexToBytes("0411"+
	...
	p2wsh, err := chainutil.NewAddressWitnessScriptHash(hexToBytes("e981bd992a43650657"+
	...
	p2tr, err := chainutil.NewAddressTaproot(hexToBytes("3a8e170b546c3b122ab9c175e"+
	...
	p2wpkh, err := chainutil.NewAddressWitnessPubKeyHash(hexToBytes("748e50366adb8"+
	...
	tests := []struct {
		...
		{(*chainutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*chainutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*chainutil.AddressPubKey)(nil), "", errUnsupportedAddress},
		{(*chainutil.AddressWitnessPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*chainutil.AddressWitnessScriptHash)(nil), "", errUnsupportedAddress},
		{(*chainutil.AddressTaproot)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*chainutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*chainutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
...
var scriptClassTests = []struct {
	...
	// codeseparator parts have been elided. (flokicoin's checks for
