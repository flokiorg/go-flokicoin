...
func pubKeyHashToAddrs(hash []byte, params *chaincfg.Params) []chainutil.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []chainutil.Address
	addr, err := chainutil.NewAddressPubKeyHash(hash, params)
	if err == nil {
...
func scriptHashToAddrs(hash []byte, params *chaincfg.Params) []chainutil.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []chainutil.Address
	addr, err := chainutil.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []chainutil.Address, int, error) {
	...
	if data := extractPubKey(pkScript); data != nil {
		var addrs []chainutil.Address
		addr, err := chainutil.NewAddressPubKey(data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if details.valid {
		...
		addrs := make([]chainutil.Address, 0, len(details.pubKeys))
		for _, pubkey := range details.pubKeys {
			addr, err := chainutil.NewAddressPubKey(pubkey, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if hash := extractWitnessPubKeyHash(pkScript); hash != nil {
		var addrs []chainutil.Address
		addr, err := chainutil.NewAddressWitnessPubKeyHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if hash := extractWitnessV0ScriptHash(pkScript); hash != nil {
		var addrs []chainutil.Address
		addr, err := chainutil.NewAddressWitnessScriptHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if rawKey := extractWitnessV1KeyBytes(pkScript); rawKey != nil {
		var addrs []chainutil.Address
		addr, err := chainutil.NewAddressTaproot(rawKey, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
