...
// parseShortForm parses a string, as used in the reference tests,
// into the script it originated from.
//
...
// TestCalcSignatureHash runs the signature hash calculation tests
// in sighash.json.
...
func executeTaprootRefTest(t *testing.T, testCase taprootJsonTest) {
	...
	tx, err := chainutil.NewTxFromBytes(txHex)
	if err != nil {
		t.Fatalf("unable to decode hex: %v", err)
	}
