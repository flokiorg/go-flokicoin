...
func main() {
	...
	method := args[0]
	usageFlags, err := chainjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := chainjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// chainjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(chainjson.Error); ok {
		...
		// The error is not a chainjson.Error and this really should not
	...
	marshalledJSON, err := chainjson.MarshalCmd(chainjson.RpcVersion1, 1, cmd)
	if err != nil {
