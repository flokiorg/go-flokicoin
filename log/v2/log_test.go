...
// BenchmarkV1vsV2Loggers compares the performance of the flog V1 logger to
// the flog V2 logger in various logs that can be used with both the legacy
...
func BenchmarkV1vsV2Loggers(b *testing.B) {
	loggers := []struct {
		...
		{
			name:   "flog v1",
			Logger: flog.NewBackend(io.Discard).Logger("V1"),
		},
		{
			name:   "flog v2",
