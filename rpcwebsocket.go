...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *chainutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *chainutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *chainutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a chainutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := chainutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a chainutil.Address) bool {
	switch a := a.(type) {
	case *chainutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a chainutil.Address) {
	switch a := a.(type) {
	case *chainutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected chainutil.Block
type notificationBlockDisconnected chainutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *chainutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	marshalledJSON, err := chainjson.MarshalCmd(chainjson.RpcVersion1, nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *chainutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *chainutil.Tx) {
	...
	ntfn := chainjson.NewTxAcceptedNtfn(txHashStr, chainutil.Amount(amount).ToFLC())
	marshalledJSON, err := chainjson.MarshalCmd(chainjson.RpcVersion1, nil, ntfn)
	if err != nil {
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			marshalledJSONVerbose, err = chainjson.MarshalCmd(chainjson.RpcVersion1, nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*chainutil.Tx)
	for _, op := range ops {
...
// blockDetails creates a BlockDetails struct to include in ws notifications
// from a block and a transaction's block index.
func blockDetails(block *chainutil.Block, txIndex int) *chainjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *chainutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := chainjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return chainjson.MarshalCmd(chainjson.RpcVersion1, nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *chainutil.Tx, block *chainutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *chainutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		...
		marshalled, err := chainjson.MarshalCmd(chainjson.RpcVersion1, nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *chainutil.Tx, block *chainutil.Block) {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				reply, err = createMarshalledReply(chainjson.RpcVersion1, nil, nil, jsonErr)
				if err != nil {
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &chainjson.RPCError{
					Code:    chainjson.ErrRPCInvalidRequest.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = chainjson.MarshalResponse(chainjson.RpcVersion2, nil, nil, jsonErr)
				if err != nil {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := chainutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *chainutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *chainutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *chainutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *chainjson.RescanCmd, lookups *rescanKeys, minBlock,
	maxBlock int32, chain *blockchain.BlockChain) (
	*chainutil.Block, *chainhash.Hash, error) {
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &chainjson.RPCError{
				Code:    chainjson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := chainjson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := chainjson.MarshalCmd(chainjson.RpcVersion1, nil, n)
			if err != nil {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	// Notify websocket client of the finished rescan.  Due to how flokicoind
	...
	// been sent.
	n := chainjson.NewRescanFinishedNtfn(
		lastBlockHash.String(), lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix(),
	)
	if mn, err := chainjson.MarshalCmd(chainjson.RpcVersion1, nil, n); err != nil {
