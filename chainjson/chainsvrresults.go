...
type GetBlockStatsResult struct {
	AverageFee         int64   `json:"avgfee"`              // Average fee in the block
	AverageFeeRate     int64   `json:"avgfeerate"`          // Average feerate (in lokis per virtual byte)
	AverageTxSize      int64   `json:"avgtxsize"`           // Average transaction size
	FeeratePercentiles []int64 `json:"feerate_percentiles"` // Feerates at the 10th, 25th, 50th, 75th, and 90th percentile weight unit (in lokis per virtual byte)
	Hash               string  `json:"blockhash"`           // The block hash (to check for potential reorgs)
	Height             int32   `json:"height"`              // The height of the block
	Ins                int64   `json:"ins"`                 // The number of inputs (excluding coinbase)
	MaxFee             int64   `json:"maxfee"`              // Maximum fee in the block
	MaxFeeRate         int64   `json:"maxfeerate"`          // Maximum feerate (in lokis per virtual byte)
	MaxTxSize          int64   `json:"maxtxsize"`           // Maximum transaction size
	MedianFee          int64   `json:"medianfee"`           // Truncated median fee in the block
	MedianTime         int64   `json:"mediantime"`          // The block median time past
	MedianTxSize       int64   `json:"mediantxsize"`        // Truncated median transaction size
	MinFee             int64   `json:"minfee"`              // Minimum fee in the block
	MinFeeRate         int64   `json:"minfeerate"`          // Minimum feerate (in lokis per virtual byte)
	MinTxSize          int64   `json:"mintxsize"`           // Minimum transaction size
	Outs               int64   `json:"outs"`                // The number of outputs
	SegWitTotalSize    int64   `json:"swtotal_size"`        // Total size of all SegWit transactions
	SegWitTotalWeight  int64   `json:"swtotal_weight"`      // Total weight of all SegWit transactions
	SegWitTxs          int64   `json:"swtxs"`               // The number of SegWit transactions
	Subsidy            int64   `json:"subsidy"`             // The block subsidy
	Time               int64   `json:"time"`                // The block time
	TotalOut           int64   `json:"total_out"`           // Total amount in all outputs (excluding coinbase and thus reward [subsidy + total fee])
	TotalSize          int64   `json:"total_size"`          // Total size of all non-coinbase transactions
	TotalWeight        int64   `json:"total_weight"`        // Total weight of all non-coinbase transactions
	Txs                int64   `json:"txs"`                 // The number of transactions (including coinbase)
	UTXOIncrease       int64   `json:"utxo_increase"`       // The increase/decrease in the number of unspent outputs
	UTXOSizeIncrease   int64   `json:"utxo_size_inc"`       // The increase/decrease in size for the UTXO index (not discounting op_return and similar)
	TotalFee           int64   `json:"totalfee"`            // The total fees in the block (not included in the original struct)
}
...
type GetMempoolInfoResult struct {
	...
	Loaded           bool    `json:"loaded"`           // True if the mempool is fully loaded
	Usage            int64   `json:"usage"`            // Total memory usage for the mempool
	MaxMempool       int64   `json:"maxmempool"`       // Maximum memory usage for the mempool
	MempoolMinFee    float64 `json:"mempoolminfee"`    // Minimum fee rate in FLC/kB for tx to be accepted. Is the maximum of minrelaytxfee and minimum mempool fee
...
type GetTxOutSetInfoResult struct {
	...
	TotalAmount    chainutil.Amount `json:"total_amount"`
}
...
func (g *GetTxOutSetInfoResult) UnmarshalJSON(data []byte) error {
	...
	amount, err := chainutil.NewAmount(aux.TotalAmount)
	if err != nil {
		return err
	}
...
// IndexInfoResult models the data returned by the chain server getindexinfo command.
type IndexInfoResult struct {
	TxIndex TxIndexResult `json:"txindex"`
}

// TxIndexResult models the tx data returned by the chain server getindexinfo command.
...
func (f *FundRawTransactionResult) UnmarshalJSON(data []byte) error {
	...
	fee, err := chainutil.NewAmount(rawRes.Fee)
	if err != nil {
		return err
	}
...
type TestMempoolAcceptFees struct {
	// Base is the transaction fee in FLC.
	Base float64 `json:"base"`

	// EffectiveFeeRate specifies the effective feerate in FLC per KvB. May
