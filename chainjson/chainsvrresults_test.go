...
func TestChainSvrCustomResults(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: &chainjson.GetZmqNotificationResult{{
				Type: "pubrawblock",
...
func TestGetTxOutSetInfoResult(t *testing.T) {
	...
	tests := []struct {
		name   string
		result string
		want   chainjson.GetTxOutSetInfoResult
	}{
		{
			...
			want: chainjson.GetTxOutSetInfoResult{
				...
				DiskSize: 1,
				TotalAmount: func() chainutil.Amount {
					a, err := chainutil.NewAmount(0.2)
					if err != nil {
						panic(err)
					}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var out chainjson.GetTxOutSetInfoResult
...
func TestChainSvrMiningInfoResults(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected chainjson.GetMiningInfoResult
	}{
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var miningInfoResult chainjson.GetMiningInfoResult
...
func TestGetNetworkInfoWarnings(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected chainjson.GetNetworkInfoResult
	}{
		{
			name:   "network info with single warning",
			result: `{"warnings": "this is a warning"}`,
			expected: chainjson.GetNetworkInfoResult{
				Warnings: chainjson.StringOrArray{
					"this is a warning",
				},
			},
		},
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var infoResult chainjson.GetNetworkInfoResult
