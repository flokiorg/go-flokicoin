...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   chainjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{chainjson.UFWalletOnly, "UFWalletOnly"},
		{chainjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{chainjson.UFNotification, "UFNotification"},
		{chainjson.UFWalletOnly | chainjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{chainjson.UFWalletOnly | chainjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := chainjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrDuplicateMethod},
		},
		{
			...
			flags: chainjson.TstHighestUsageFlagBit,
			err:   chainjson.Error{ErrorCode: chainjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrEmbeddedType},
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrUnexportedField},
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrNonOptionalField},
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrNonOptionalDefault},
		},
		{
			...
			err: chainjson.Error{ErrorCode: chainjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := chainjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	chainjson.MustRegisterCmd("panicme", 0, 0)
}
