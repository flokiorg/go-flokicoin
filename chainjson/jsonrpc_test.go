...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if chainjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *chainjson.RPCError {
				return chainjson.NewRPCError(chainjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := chainjson.MarshalResponse(chainjson.RpcVersion1, testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := chainjson.NewRequest(chainjson.RpcVersion1, nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := chainjson.Error{ErrorCode: chainjson.ErrInvalidType}
	_, err = chainjson.MarshalResponse(chainjson.RpcVersion1, make(chan int), nil, nil)
	if jerr, ok := err.(chainjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = chainjson.MarshalResponse(chainjson.RpcVersion1, 1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		in   *chainjson.RPCError
		want string
	}{
		{
			chainjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			chainjson.ErrRPCMethodNotFound,
