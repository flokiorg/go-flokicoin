...
func TestAssignField(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			dest:     chainjson.Int32(0),
			src:      "null",
			expected: nil,
		},
		{
			name:     `null optional field - "null" -> *string`,
			dest:     chainjson.String(""),
			src:      "null",
			expected: nil,
		},
	}
	...
	for i, test := range tests {
		...
		err := chainjson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			args:   []interface{}{},
			err:    chainjson.Error{ErrorCode: chainjson.ErrUnregisteredMethod},
		},
		{
			name:   "too few parameters to command with required + optional",
			method: "getblock",
			args:   []interface{}{},
			err:    chainjson.Error{ErrorCode: chainjson.ErrNumParams},
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := chainjson.NewCmd(test.method, test.args...)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestMarshalCmd(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		bytes, err := chainjson.MarshalCmd(chainjson.RpcVersion1, test.id, test.cmd)
		if err != nil {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v)",
				i, test.name, err, err)
			continue
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			id:   1,
			cmd:  (*chainjson.GetBlockCmd)(nil),
			err:  chainjson.Error{ErrorCode: chainjson.ErrInvalidType},
		},
		{
			name: "nil instance of registered type",
			id:   []int{0, 1},
			cmd:  &chainjson.GetBlockCountCmd{},
			err:  chainjson.Error{ErrorCode: chainjson.ErrInvalidType},
		},
	}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := chainjson.MarshalCmd(chainjson.RpcVersion1, test.id, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		name    string
		request chainjson.Request
		err     chainjson.Error
	}{
		{
			name: "unregistered type",
			request: chainjson.Request{
				Jsonrpc: chainjson.RpcVersion1,
				Method:  "bogusmethod",
				Params:  nil,
				ID:      nil,
			},
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := chainjson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
