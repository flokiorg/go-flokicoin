...
// FLCPerkvB is the units used to represent Flokicoin transaction fees.
// This unit represents the fee in FLC for a transaction size of 1 kB.
type FLCPerkvB = float64
...
type CreateRawTransactionCmd struct {
	...
	Amounts  map[string]float64 `jsonrpcusage:"{\"address\":amount,...}"` // In FLC
	LockTime *int64
}
...
//
// Amounts are in FLC. Passing in nil and the empty slice as inputs is equivalent,
...
type FundRawTransactionOpts struct {
	...
	FeeRate                *FLCPerkvB            `json:"feeRate,omitempty"` // FLC/kB
...
type TestMempoolAcceptCmd struct {
	...
	// value, expressed in FLC/kvB, optional, default="0.10".
	MaxFeeRate FLCPerkvB `json:"omitempty"`
}
...
func NewTestMempoolAcceptCmd(rawTxns []string,
	maxFeeRate FLCPerkvB) *TestMempoolAcceptCmd {
