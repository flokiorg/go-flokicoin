...
var signatureTests = []signatureTest{
	// signatures from flokicoin blockchain tx
	...
	// at all. When not parsing DER signatures, which is done by by flokicoind
...
func TestSignatureSerialize(t *testing.T) {
	tests := []struct {
		...
		// signature from flokicoin blockchain tx
		// 0437cd7f8525ceed2324359c2d0ba26006d92d85
		{
		...
		{
			"zero signature",
			NewSignature(&crypto.ModNScalar{}, &crypto.ModNScalar{}),
...
func testSignCompact(t *testing.T, tag string, curve *crypto.KoblitzCurve,
...
func TestSignCompact(t *testing.T) {
	for i := 0; i < 256; i++ {
		...
		compressed := i%2 != 0
		testSignCompact(t, name, crypto.S256(), data, compressed)
	}
}
...
func TestRecoverCompact(t *testing.T) {
	for i, test := range recoveryTests {
		...
		if (test.err == nil) != (err == nil) || (test.err != nil && err != nil && test.err.Error() != err.Error()) {
			if test.err != nil {
				t.Logf("got=%s", err.Error())
				t.Logf("want=%s", test.err.Error())
			}
		...
		exPub, _ := crypto.ParsePubKey(decodeHex(test.pub))
...
func TestRFC6979(t *testing.T) {
	...
	for i, test := range tests {
		privKey, _ := crypto.PrivKeyFromBytes(decodeHex(test.key))
		hash := sha256.Sum256([]byte(test.msg))

		// Ensure deterministically generated nonce is the expected value.
		gotNonce := crypto.NonceRFC6979(privKey.Serialize(), hash[:], nil, nil, 0).Bytes()
...
func testSignAndVerify(t *testing.T, c *crypto.KoblitzCurve, tag string) {
	priv, _ := crypto.NewPrivateKey()
	pub := priv.PubKey()
...
func TestSignAndVerify(t *testing.T) {
	testSignAndVerify(t, crypto.S256(), "S256")
}
...
func TestPrivKeys(t *testing.T) {
	...
	for _, test := range tests {
		priv, pub := crypto.PrivKeyFromBytes(test.key)

		_, err := crypto.ParsePubKey(pub.SerializeUncompressed())
		if err != nil {
