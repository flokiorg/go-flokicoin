...
func hexToModNScalar(s string) *crypto.ModNScalar {
	b, err := hex.DecodeString(s)
	if err != nil {
		panic("invalid hex in source file: " + s)
	}
	var scalar crypto.ModNScalar
...
func hexToFieldVal(s string) *crypto.FieldVal {
	b, err := hex.DecodeString(s)
	if err != nil {
		panic("invalid hex in source file: " + s)
	}
	var f crypto.FieldVal
