...
type sortableKeys []*crypto.PublicKey
...
// serialization.
func sortKeys(keys []*crypto.PublicKey) []*crypto.PublicKey {
...
func keyHashFingerprint(keys []*crypto.PublicKey, sort bool) []byte {
	if sort {
		keys = sortKeys(keys)
	}
...
func keyBytesEqual(a, b *crypto.PublicKey) bool {
...
func aggregationCoefficient(keySet []*crypto.PublicKey,
	targetKey *crypto.PublicKey, keysHash []byte,
	secondKeyIdx int) *crypto.ModNScalar {

	var mu crypto.ModNScalar
...
func secondUniqueKeyIndex(keySet []*crypto.PublicKey, sort bool) int {
	if sort {
...
func hasEvenY(pJ crypto.JacobianPoint) bool {
	pJ.ToAffine()
	p := crypto.NewPublicKey(&pJ.X, &pJ.Y)
...
// tweak or not.
func tweakKey(keyJ crypto.JacobianPoint, parityAcc crypto.ModNScalar, tweak [32]byte,
	tweakAcc crypto.ModNScalar,
	xOnly bool) (crypto.JacobianPoint, crypto.ModNScalar, crypto.ModNScalar, error) {
	...
	var parityFactor crypto.ModNScalar
	...
	// manipulations below.
	tweakInt := new(crypto.ModNScalar)
	...
	//
	// First compute t*G:
	var tweakedGenerator crypto.JacobianPoint
	crypto.ScalarBaseMultNonConst(tweakInt, &tweakedGenerator)

	// Next compute g*Q:
	crypto.ScalarMultNonConst(&parityFactor, &keyJ, &keyJ)

	// Finally add both of them together to get our final
	// tweaked point.
	crypto.AddNonConst(&tweakedGenerator, &keyJ, &keyJ)
...
func AggregateKeys(keys []*crypto.PublicKey, sort bool,
	keyOpts ...KeyAggOption) (
	*AggregateKey, *crypto.ModNScalar, *crypto.ModNScalar, error) {
	...
	finalKeyJ.ToAffine()
	combinedKey := crypto.NewPublicKey(&finalKeyJ.X, &finalKeyJ.Y)
	...
	finalKeyJ.ToAffine()
	finalKey := crypto.NewPublicKey(&finalKeyJ.X, &finalKeyJ.Y)
