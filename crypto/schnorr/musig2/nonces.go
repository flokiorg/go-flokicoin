...
func secNonceToPubNonce(secNonce [SecNonceSize]byte) [PubNonceSize]byte {
	var k1Mod, k2Mod crypto.ModNScalar
	k1Mod.SetByteSlice(secNonce[:crypto.PrivKeyBytesLen])
	k2Mod.SetByteSlice(secNonce[crypto.PrivKeyBytesLen:])

	var r1, r2 crypto.JacobianPoint
	crypto.ScalarBaseMultNonConst(&k1Mod, &r1)
	crypto.ScalarBaseMultNonConst(&k2Mod, &r2)

	// Next, we'll convert the key in jacobian format to a normal public
	// key expressed in affine coordinates.
	r1.ToAffine()
	r2.ToAffine()
	r1Pub := crypto.NewPublicKey(&r1.X, &r1.Y)
	r2Pub := crypto.NewPublicKey(&r2.X, &r2.Y)
...
// generation.
func WithPublicKey(pubKey *crypto.PublicKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
func WithNonceSecretKeyAux(secKey *crypto.PrivateKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
// generate nonces.
func WithNonceCombinedKeyAux(combinedKey *crypto.PublicKey) NonceGenOption {
	return func(o *nonceGenOpts) {
...
// *crypto.PrivateKey. This is mainly used to match the testcases provided by
// the MuSig2 BIP.
...
func GenNonces(options ...NonceGenOption) (*Nonces, error) {
	...
	k2Mod.PutBytesUnchecked(nonces.SecNonce[crypto.PrivKeyBytesLen:])
	copy(nonces.SecNonce[crypto.PrivKeyBytesLen*2:], opts.publicKey)
...
func AggregateNonces(pubNonces [][PubNonceSize]byte) ([PubNonceSize]byte, error) {
	...
	combineNonces := func(slicer nonceSlicer) (crypto.JacobianPoint, error) {
		...
		var aggregateNonce crypto.JacobianPoint
	...
	combinedNonce1, err := combineNonces(func(n [PubNonceSize]byte) []byte {
		return n[:crypto.PubKeyBytesLenCompressed]
	})
	if err != nil {
	...
	copy(finalNonce[:], crypto.JacobianToByteSlice(combinedNonce1))
	copy(
		finalNonce[crypto.PubKeyBytesLenCompressed:],
		crypto.JacobianToByteSlice(combinedNonce2),
	)
