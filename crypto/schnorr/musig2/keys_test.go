...
func TestMusig2KeySort(t *testing.T) {
	...
	keys := make([]*crypto.PublicKey, len(testCase.PubKeys))
	for i, keyStr := range testCase.PubKeys {
		pubKey, err := crypto.ParsePubKey(mustParseHex(keyStr))
		require.NoError(t, err)
	...
	expectedKeys := make([]*crypto.PublicKey, len(testCase.PubKeys))
...
func keysFromIndices(t *testing.T, indices []int,
	pubKeys []string) ([]*crypto.PublicKey, error) {

	t.Helper()

	inputKeys := make([]*crypto.PublicKey, len(indices))
	for i, keyIdx := range indices {
		var err error
		inputKeys[i], err = crypto.ParsePubKey(
...
func TestMuSig2TweakTestVectors(t *testing.T) {
	...
	privKey, _ := crypto.PrivKeyFromBytes(mustParseHex(testCases.PrivKey))
