...
func (s signerSet) keys() []*crypto.PublicKey {
	keys := make([]*crypto.PublicKey, len(s))
	for i := 0; i < len(s); i++ {
		keys[i] = s[i].pubKey
	}
...
func (s signerSet) combinedKey() *crypto.PublicKey {
...
func testMultiPartySign(t *testing.T, taprootTweak []byte,
	...
	signerKeys := make([]*crypto.PrivateKey, numSigners)
	signSet := make([]*crypto.PublicKey, numSigners)
	...
	var combinedKey *crypto.PublicKey
...
func TestMuSigEarlyNonce(t *testing.T) {
	...
	signers := []*crypto.PublicKey{privKey1.PubKey(), privKey2.PubKey()}
...
func TestSigningWithAggregatedNonce(t *testing.T) {
	...
	t.Run("error: register combined nonce twice", func(t *testing.T) {
		...
		privKey2, _ := crypto.NewPrivateKey()
		signSet := []*crypto.PublicKey{privKey.PubKey(), privKey2.PubKey()}
	...
	t.Run("error: register combined nonce after register pub nonce",
		func(t *testing.T) {
			...
			privKey, _ := crypto.NewPrivateKey()
			privKey2, _ := crypto.NewPrivateKey()
			privKey3, _ := crypto.NewPrivateKey()
			signSet := []*crypto.PublicKey{
				privKey.PubKey(),
				privKey2.PubKey(),
				privKey3.PubKey(),
			}
