...
var infinityPoint crypto.JacobianPoint
...
func NewPartialSignature(s *crypto.ModNScalar,
	r *crypto.PublicKey) PartialSignature {
...
func computeSigningNonce(combinedNonce [PubNonceSize]byte,
	combinedKey *crypto.PublicKey, msg [32]byte) (
	*crypto.JacobianPoint, *crypto.ModNScalar, error) {
	...
	var (
		...
		nonceBlinder crypto.ModNScalar
	)
	...
	r1J, err := crypto.ParseJacobian(
		combinedNonce[:crypto.PubKeyBytesLenCompressed],
	)
	if err != nil {
		return nil, nil, err
	}
	r2J, err := crypto.ParseJacobian(
		combinedNonce[crypto.PubKeyBytesLenCompressed:],
	)
	if err != nil {
		return nil, nil, err
	...
	//  * R = R_1 + b*R_2
	var nonce crypto.JacobianPoint
	crypto.ScalarMultNonConst(&nonceBlinder, &r2J, &r2J)
	crypto.AddNonConst(&r1J, &r2J, &nonce)
...
// infinity.
func Sign(secNonce [SecNonceSize]byte, privKey *crypto.PrivateKey,
	combinedNonce [PubNonceSize]byte, pubKeys []*crypto.PublicKey,
	...
	if !bytes.Equal(secNonce[crypto.PrivKeyBytesLen*2:],
	...
	var k1, k2 crypto.ModNScalar
	k1.SetByteSlice(secNonce[:crypto.PrivKeyBytesLen])
	k2.SetByteSlice(secNonce[crypto.PrivKeyBytesLen:])
	...
	nonceKey := crypto.NewPublicKey(&nonce.X, &nonce.Y)
	...
	parityCombinedKey := new(crypto.ModNScalar).SetInt(1)
...
func (p *PartialSignature) Verify(pubNonce [PubNonceSize]byte,
	combinedNonce [PubNonceSize]byte, keySet []*crypto.PublicKey,
	signingKey *crypto.PublicKey, msg [32]byte, signOpts ...SignOption) bool {
...
func verifyPartialSig(partialSig *PartialSignature, pubNonce [PubNonceSize]byte,
	...
	pubNonce1J, err := crypto.ParseJacobian(
		pubNonce[:crypto.PubKeyBytesLenCompressed],
	)
	if err != nil {
		return err
	}
	pubNonce2J, err := crypto.ParseJacobian(
		pubNonce[crypto.PubKeyBytesLenCompressed:],
	)
	if err != nil {
		return err
	...
	if nonce == infinityPoint {
		crypto.GeneratorJacobian(&nonce)
	} else {
		nonce.ToAffine()
	}
	...
	var pubNonceJ crypto.JacobianPoint

	crypto.ScalarMultNonConst(&nonceBlinder, &pubNonce2J, &pubNonce2J)
	crypto.AddNonConst(&pubNonce1J, &pubNonce2J, &pubNonceJ)
	...
	//  * e = H(tag=ChallengeHashTag, R || Q || m) mod n
	var challengeMsg bytes.Buffer
	challengeMsg.Write(schnorr.SerializePubKey(crypto.NewPublicKey(
		&nonce.X, &nonce.Y,
	)))
	challengeMsg.Write(schnorr.SerializePubKey(combinedKey.FinalKey))
	challengeMsg.Write(msg[:])
	...
	signingKey, err := crypto.ParsePubKey(pubKey)
	if err != nil {
		return err
	}
	...
	var signKeyJ crypto.JacobianPoint
	signingKey.AsJacobian(&signKeyJ)

	// In the final set, we'll check that: s*G == R' + e*a*g*P.
	var sG, rP crypto.JacobianPoint
	crypto.ScalarBaseMultNonConst(s, &sG)
	crypto.ScalarMultNonConst(e.Mul(a).Mul(finalParityFactor), &signKeyJ, &rP)
	crypto.AddNonConst(&rP, &pubNonceJ, &rP)
...
type combineOptions struct {
	...
	combinedKey *crypto.PublicKey

	tweakAcc *crypto.ModNScalar
}
...
func WithTweakedCombine(msg [32]byte, keys []*crypto.PublicKey,
...
func WithTaprootTweakedCombine(msg [32]byte, keys []*crypto.PublicKey,
...
func WithBip86TweakedCombine(msg [32]byte, keys []*crypto.PublicKey,
...
func CombineSigs(combinedNonce *crypto.PublicKey,
	...
	var tweakProduct *crypto.ModNScalar
	if opts.combinedKey != nil && opts.tweakAcc != nil {
		...
		parityFactor := new(crypto.ModNScalar).SetInt(1)
	...
	var nonceJ crypto.JacobianPoint
