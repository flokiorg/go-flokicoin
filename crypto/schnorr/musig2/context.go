...
func WithKnownSigners(signers []*crypto.PublicKey) ContextOption {
	return func(o *contextOptions) {
		o.keySet = signers
...
func NewContext(signingKey *crypto.PrivateKey, shouldSort bool,
	...
	case opts.numSigners != 0:
		...
		// be created.
		opts.keySet = make([]*crypto.PublicKey, 0, opts.numSigners)
...
func (c *Context) RegisterSigner(pub *crypto.PublicKey) (bool, error) {
...
func (c *Context) CombinedKey() (*crypto.PublicKey, error) {
...
func (c *Context) PubKey() crypto.PublicKey {
	return *c.pubKey
}

// SigningKeys returns the set of keys used for signing.
func (c *Context) SigningKeys() []*crypto.PublicKey {
	keys := make([]*crypto.PublicKey, len(c.opts.keySet))
...
func (c *Context) TaprootInternalKey() (*crypto.PublicKey, error) {
	// If the caller hasn't registered all the signers at this point, then
	// the combined key won't be available.
	if c.combinedKey == nil {
		return nil, ErrNotEnoughSigners
...
func (s *Session) RegisterCombinedNonce(
	...
	// ensure it's valid.
	_, err := crypto.ParsePubKey(combinedNonce[:33])
	if err != nil {
