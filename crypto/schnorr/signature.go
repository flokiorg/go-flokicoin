...
func NewSignature(r *crypto.FieldVal, s *crypto.ModNScalar) *Signature {
	var sig Signature
...
func schnorrVerify(sig *Signature, hash []byte, pubKeyBytes []byte) error {
	...
	//
	// R = s*G - e*P
	var P, R, sG, eP crypto.JacobianPoint
	pubKey.AsJacobian(&P)
	crypto.ScalarBaseMultNonConst(&sig.s, &sG)
	crypto.ScalarMultNonConst(&e, &P, &eP)
	crypto.AddNonConst(&sG, &eP, &R)
...
func (sig *Signature) Verify(hash []byte, pubKey *crypto.PublicKey) bool {
...
func schnorrSign(privKey, nonce *crypto.ModNScalar, pubKey *crypto.PublicKey, hash []byte,
	...
	//
	// R = kG
	var R crypto.JacobianPoint
	k := *nonce
	crypto.ScalarBaseMultNonConst(&k, &R)
	...
	//
	// s = k + e*d mod n
	s := new(crypto.ModNScalar).Mul2(&e, privKey).Add(&k)
	k.Zero()
...
func Sign(privKey *crypto.PrivateKey, hash []byte,
	...
	//
	// d' = int(d)
	var privKeyScalar crypto.ModNScalar
