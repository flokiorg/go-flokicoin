...
import (
	...
	"github.com/flokiorg/go-flokicoin/chainutil/bloom"
	...
	"github.com/flokiorg/go-flokicoin/mining/cpuminer"
	"github.com/flokiorg/go-flokicoin/netaddr"
	"github.com/flokiorg/go-flokicoin/netsync"
...
// broadcastMsg provides the ability to house a flokicoin message to be broadcast
...
// server provides a flokicoin server for handling communications to and from
// flokicoin peers.
type server struct {
	...
	addrManager          *netaddr.AddrManager
...
func (sp *serverPeer) addKnownAddresses(addresses []*wire.NetAddressV2) {
	...
	for _, na := range addresses {
		sp.knownAddresses.Add(netaddr.NetAddressKey(na))
	}
...
func (sp *serverPeer) addressKnown(na *wire.NetAddressV2) bool {
	...
	exists := sp.knownAddresses.Contains(netaddr.NetAddressKey(na))
...
// OnTx is invoked when a peer receives a tx flokicoin message.  It blocks
// until the flokicoin transaction has been fully processed.  Unlock the block
...
// OnBlock is invoked when a peer receives a block flokicoin message.  It
// blocks until the flokicoin block has been fully processed.
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	...
	// until the flokicoin block is fully processed and known
	...
	// the flokicoin block has been fully processed.
	sp.server.syncManager.QueueBlock(block, sp.Peer, sp.blockProcessed)
	<-sp.blockProcessed
}

// OnInv is invoked when a peer receives an inv flokicoin message and is
...
// OnHeaders is invoked when a peer receives a headers flokicoin
// message.  The message is passed down to the sync manager.
func (sp *serverPeer) OnHeaders(_ *peer.Peer, msg *wire.MsgHeaders) {
	sp.server.syncManager.QueueHeaders(msg, sp.Peer)
}

// handleGetData is invoked when a peer receives a getdata flokicoin message and
...
// OnGetCFilters is invoked when a peer receives a getcfilters flokicoin message.
...
// OnGetCFHeaders is invoked when a peer receives a getcfheader flokicoin message.
...
// OnGetCFCheckpt is invoked when a peer receives a getcfcheckpt flokicoin message.
...
// OnFeeFilter is invoked when a peer receives a feefilter flokicoin message and
// is used by remote peers to request that no transactions which have a fee rate
// lower than provided value are inventoried to them.  The peer will be
// disconnected if an invalid fee filter value is provided.
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	// Check that the passed minimum fee is a valid amount.
	if msg.MinFee < 0 || msg.MinFee > chainutil.MaxLoki {
...
// OnFilterAdd is invoked when a peer receives a filteradd flokicoin
...
// OnAddr is invoked when a peer receives an addr flokicoin message and is
...
// OnAddrV2 is invoked when a peer receives an addrv2 flokicoin message and is
// used to notify the server about advertised addresses.
	...
	for {
		...
		case <-timer.C:
			...
			if first && err == nil {
				...
				err = s.addrManager.AddLocalAddress(na, netaddr.UpnpPrio)
				if err != nil {
					// XXX DeletePortMapping?
				}
				srvrLog.Warnf("Successfully bound via UPnP to %s", netaddr.NetAddressKey(na))
				first = false
			}
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := netaddr.New(cfg.DataDir, lookup)
	...
	if cfg.Prune != 0 {
		flcdLog.Infof("Prune set to %d MiB", cfg.Prune)
	}
	...
	if !cfg.SimNet && len(cfg.ConnectPeers) == 0 {
		newAddressFunc = func() (net.Addr, error) {
			for tries := 0; tries < 100; tries++ {
				...
				// others.
				key := netaddr.GroupKey(addr.NetAddress())
				...
				addrString := netaddr.NetAddressKey(addr.NetAddress())
...
func initListeners(amgr *netaddr.AddrManager, listenAddrs []string, services wire.ServiceFlag) ([]net.Listener, NAT, error) {
	...
	if len(cfg.ExternalIPs) != 0 {
		...
		for _, sip := range cfg.ExternalIPs {
			...
			err = amgr.AddLocalAddress(na, netaddr.ManualPrio)
			if err != nil {
...
func addLocalAddress(addrMgr *netaddr.AddrManager, addr string, services wire.ServiceFlag) error {
	...
	if ip := net.ParseIP(host); ip != nil && ip.IsUnspecified() {
		...
		for _, addr := range addrs {
			...
			nAddr := wire.NetAddressV2FromBytes(
				time.Now(), services, ifaceIP, uint16(port),
			)
			addrMgr.AddLocalAddress(nAddr, netaddr.BoundPrio)
		}
	} else {
		...
		addrMgr.AddLocalAddress(netAddr, netaddr.BoundPrio)
	}
