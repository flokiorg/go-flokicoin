...
func TestMsgAlert(t *testing.T) {
	...
	// Test FlcEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.FlcEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("FlcEncode got: %s want: %s",
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.FlcEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.FlcEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.FlcEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.FlcEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
...
func TestAlert(t *testing.T) {
	...
	alert := NewAlert(
		...
		1013, []int32{1014}, 0, 40599, []string{"/Loki:0.7.2/"}, 5000, "",
