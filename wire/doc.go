...
For the complete details of the flokicoin protocol, see the official wiki entry
...
supported flokicoin messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

# Flokicoin Message Overview

The flokicoin protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which flokicoin network it is a part of, its type, how big it is, and a checksum
to verify validity.  All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for flokicoin messages named
...
of the currently supported flokicoin messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using flokicoin encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the flokicoin messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official flokicoin protocol wiki entry at
...
and write flokicoin messages.  The following sections provide a quick overview of
...
The flokicoin network is a magic number which is used to identify the start of a
message and which flokicoin network the message applies to.  This package provides
...
As discussed in the flokicoin message overview section, this package reads
and writes flokicoin messages using a generic interface named Message.  In
...
In order to unmarshall flokicoin messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a flokicoin peer.  Example syntax is:

	// Reads and validates the next flokicoin message from conn using the
	// protocol version pver and the flokicoin network flcnet.  The returns
	// are a wire.Message, a []byte which contains the unmarshalled
	// raw payload, and a possible error.
	msg, rawPayload, err := wire.ReadMessage(conn, pver, flcnet)
	if err != nil {
...
In order to marshall flokicoin messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a flokicoin peer.  Example syntax to request addresses
from a remote peer is:

	// Create a new getaddr flokicoin message.
	msg := wire.NewMsgGetAddr()

	// Writes a flokicoin message msg to conn using the protocol version
	// pver, and the flokicoin network flcnet.  The return is a possible
	// error.
	err := wire.WriteMessage(conn, msg, pver, flcnet)
	if err != nil {
		// Log and handle the error
	}
...
# Flokicoin Improvement Proposals
