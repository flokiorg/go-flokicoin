...
// MsgCFilter implements the Message interface and represents a flokicoin cfilter
...
func (msg *MsgCFilter) FlcDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFilter) FlcEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	size := len(msg.Data)
	if size > MaxCFilterDataSize {
		str := fmt.Sprintf("cfilter size too large for message "+
			"[size %v, max %v]", size, MaxCFilterDataSize)
		return messageError("MsgCFilter.FlcEncode", str)
	}
...
// from FlcDecode in that FlcDecode decodes from the flokicoin wire protocol as
...
// NewMsgCFilter returns a new flokicoin cfilter message that conforms to the
