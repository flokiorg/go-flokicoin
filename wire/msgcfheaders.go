...
const (
	...
	// that can be in a single flokicoin cfheaders message.
	MaxCFHeadersPerMsg = 2000
)

// MsgCFHeaders implements the Message interface and represents a flokicoin
...
func (msg *MsgCFHeaders) FlcDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.FlcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) FlcEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	count := len(msg.FilterHashes)
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.FlcEncode", str)
	}
...
// differs from FlcDecode in that FlcDecode decodes from the flokicoin wire
...
// NewMsgCFHeaders returns a new flokicoin cfheaders message that conforms to
