...
// MsgVersion implements the Message interface and represents a flokicoin version
...
func (msg *MsgVersion) FlcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if !ok {
		return fmt.Errorf("MsgVersion.FlcDecode reader is not a " +
			"*bytes.Buffer")
	}
...
// NewMsgVersion returns a new flokicoin version message that conforms to the
