...
func TestAuxPowHeader(t *testing.T) {

	auxpow := ""
	raw, err := hex.DecodeString(auxpow)
	if err != nil {
		t.Fatal(err)
	}

	var aph AuxPowHeader
	if err := aph.Deserialize(bytes.NewReader(raw)); err != nil {
		t.Fatal(err)
	}

	t.Logf("aph: %s", aph.String())
	t.Logf("parentBlockHash.pow: %v", aph.ParentBlockHeader.BlockPoWHash())
	t.Logf("parentBlockHash: %v", aph.ParentBlockHeader.BlockHash())
...
func TestAuxPow_MultiTx_CoinbaseBranchOnly(t *testing.T) {
    ...
    // Build coinbase script with header and auxRoot and params.
    script := makeMMCoinbaseScript(auxRoot[:], mSize, mNonce, true, 0, false)
    ...
    // Compute expectedIndex using current implementation (uses mSize as height).
    expectedIndex := getExpectedIndex(mNonce, uint32(chainID), mSize)
    ...
    // rev(ParentBlockHash) must equal header.BlockHash()
    hdrHash := pHdr.BlockHash()
    rev32 := rev(hdrHash)
    aph.ParentBlockHash = mustHashFromHex(t, hex.EncodeToString(rev32[:]))
...
//
//	header (optional) || auxRootLE || mSize(LE u32) || mNonce(LE u32)
//
...
func makeMMCoinbaseScript(auxRootLE []byte, mSize, mNonce uint32, withHeader bool, forceOffset int, dupHeader bool) []byte {
	...
	body.Write(auxRootLE)     // aux root (LE) must follow header immediately when present
...
func mkAuxPow(t *testing.T, childHash chainhash.Hash, chainID int32, opts auxPowOpts) *AuxPowHeader {
	...
	// SideMask must equal getExpectedIndex(mNonce, chainID, mSize) with your current signature.
	...
	// Aux root used in coinbase payload is little-endian bytes of child hash (after branch folding).
	auxRootLE := make([]byte, 32)
	copy(auxRootLE, childHash[:]) // childHash is already internal LE; keep as-is for payload
	// coinbase script
	cmSize := mSize
	if opts.wrongCoinbaseMSz != nil {
		cmSize = *opts.wrongCoinbaseMSz
	}
	script := makeMMCoinbaseScript(auxRootLE, cmSize, mNonce, opts.withHeader, opts.forceOffset, opts.dupHeader)
	cbTx := mkCoinbaseTx(script)
	...
	aph := &AuxPowHeader{
		CoinbaseTx:        cbTx,
		ParentBlockHash:   chainhash.Hash{},                       // fill below
	...
	// Set ParentBlockHash to match check:
	// rev(ParentBlockHash) must equal header.BlockHash()
	hdrHash := pHdr.BlockHash()
	if !opts.mismatchParentHash {
		...
		// Deliberately mismatch: tweak one byte
		r := rev(hdrHash)
		r[0] ^= 0x01
		aph.ParentBlockHash = mustHashFromHex(t, hex.EncodeToString(r[:]))
	}
