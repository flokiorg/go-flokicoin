...
func TestVersion(t *testing.T) {
	...
	tcpAddrMe := &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 15212}
	me := NewNetAddress(tcpAddrMe, SFNodeNetwork)
	tcpAddrYou := &net.TCPAddr{IP: net.ParseIP("192.168.0.1"), Port: 15212}
...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.FlcDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.FlcDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.FlcDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("FlcEncode #%d wrong error got: %v, "+
		...
		err = msg.FlcDecode(buf, test.pver, test.enc)
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("FlcDecode #%d wrong error got: %v, "+
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/floktest:0.0.1/"
...
var baseVersionEncoded = []byte{
	...
	0x3b, 0x6c, // Port 15212 in big-endian
	...
	0x2F, 0x66, 0x6C, 0x6F, 0x6B, 0x74, 0x65, 0x73, 0x74, 0x3A,
	0x30, 0x2E, 0x30, 0x2E, 0x31, 0x2F, // User agent
