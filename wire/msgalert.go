...
// subversion would typically be something like "/Loki:0.7.2/" (15 bytes)
...
// MsgAlert  implements the Message interface and defines a flokicoin alert
// message.
//
// This is a signed message that provides notifications that the client should
// display if the signature matches the key.
...
func (msg *MsgAlert) FlcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	var err error
...
// This is part of the Message interface implementation.
func (msg *MsgAlert) FlcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	var err error
	...
	if slen == 0 {
		return messageError("MsgAlert.FlcEncode", "empty serialized payload")
	}
...
// NewMsgAlert returns a new flokicoin alert message that conforms to the Message
