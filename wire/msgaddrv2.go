...
// MsgAddrV2 implements the Message interface and represents a flokicoin addrv2
...
// FlcDecode decodes r using the flokicoin protocol into a MsgAddrV2.
func (m *MsgAddrV2) FlcDecode(r io.Reader, pver uint32,
	...
	if count > MaxV2AddrPerMsg {
		...
		return messageError("MsgAddrV2.FlcDecode", str)
	}
...
// FlcEncode encodes the MsgAddrV2 into a writer w.
func (m *MsgAddrV2) FlcEncode(w io.Writer, pver uint32,
	enc MessageEncoding) error {
	...
	if count > MaxV2AddrPerMsg {
		str := fmt.Sprintf("too many addresses for message [count %v,"+
			" max %v]", count, MaxV2AddrPerMsg)
		return messageError("MsgAddrV2.FlcEncode", str)
	}
...
// NewMsgAddrV2 returns a new flokicoin addrv2 message that conforms to the
// Message interface.
