...
// TxFlagMarker is the first byte of the FLAG field in a flokicoin tx
...
//
// Position of FLAG in a flokicoin tx message:
//
//	┌─────────┬────────────────────┬─────────────┬─────┐
...
// TxFlag is the second byte of the FLAG field in a flokicoin tx message.
...
// NewOutPointFromString returns a new flokicoin transaction outpoint parsed from
...
// MsgTx implements the Message interface and represents a flokicoin tx message.
...
func (msg *MsgTx) FlcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	err := msg.flcDecode(r, pver, enc, buf, sbuf[:])
	return err
}

func (msg *MsgTx) flcDecode(r io.Reader, pver uint32, enc MessageEncoding,
	buf, sbuf []byte) error {
...
// the Version field in the transaction.  This function differs from FlcDecode
// in that FlcDecode decodes from the flokicoin wire protocol as it was sent
...
func (msg *MsgTx) FlcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	err := msg.flcEncode(w, pver, enc, buf)
	return err
}

func (msg *MsgTx) flcEncode(w io.Writer, pver uint32, enc MessageEncoding,
	buf []byte) error {
...
// the transaction.  This function differs from FlcEncode in that FlcEncode
// encodes the transaction to the flokicoin wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
func (msg *MsgTx) Serialize(w io.Writer) error {
	...
	//
	// Passing a encoding type of WitnessEncoding to FlcEncode for MsgTx
...
// WriteOutPoint encodes op to the flokicoin protocol encoding for an OutPoint to
// w.
...
// writeOutPointBuf encodes op to the flokicoin protocol encoding for an OutPoint
// to w.
//
// If b is non-nil, the provided buffer will be used for serializing small
// values.  Otherwise a buffer will be drawn from the binarySerializer's pool
...
// writeTxInBuf encodes ti to the flokicoin protocol encoding for a transaction
...
// WriteTxOut encodes to into the flokicoin protocol encoding for a transaction
// output (TxOut) to w.
//
...
// WriteTxOutBuf encodes to into the flokicoin protocol encoding for a transaction
...
// writeTxWitnessBuf encodes the flokicoin protocol encoding for a transaction
