...
// obtained from Flokicoin, so all credits for the algorithm go to it.
//
...
func isPubKey(script []byte) (bool, []byte) {
	...
	if len(script) == 35 && script[0] == txscript.OP_DATA_33 &&
		...
		_, err := crypto.ParsePubKey(serializedPubKey)
		if err == nil {
...
func decompressScript(compressedPkScript []byte) []byte {
	...
	case cstPayToPubKeyUncomp4, cstPayToPubKeyUncomp5:
		...
		key, err := crypto.ParsePubKey(compressedKey)
		if err != nil {
			return nil
		}
...
// Flokicoin, so all credits for the algorithm go to it.
//
// While this is simply exchanging one uint64 for another, the resulting value
// for typical amounts has a much smaller magnitude which results in fewer bytes
// when encoded as variable length quantity.  For example, consider the amount
// of 0.1 FLC which is 10000000 loki.  Encoding 10000000 as a VLQ would take
...
//            0 (1) -> 0        (1)           *  0.00000000 FLC
//         1000 (2) -> 4        (1)           *  0.00001000 FLC
//        10000 (2) -> 5        (1)           *  0.00010000 FLC
//     12345678 (4) -> 111111101(4)           *  0.12345678 FLC
//     50000000 (4) -> 47       (1)           *  0.50000000 FLC
//    100000000 (4) -> 9        (1)           *  1.00000000 FLC
//    500000000 (5) -> 49       (1)           *  5.00000000 FLC
//   1000000000 (5) -> 10       (1)           * 10.00000000 FLC
// -----------------------------------------------------------------------------
