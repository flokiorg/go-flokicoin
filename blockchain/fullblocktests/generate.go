...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := crypto.PrivKeyFromBytes([]byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := chainutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*chainutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, chainutil.NewTx(tx))
	}
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.FlcEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.FlcEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	acceptBlock := func(blockName string, block *wire.MsgBlock, isMainChain, isOrphan bool) TestInstance {
		...
		return AcceptedBlock{blockName, block, blockHeight, isMainChain, isOrphan}
	}
	...
	blockDisconnectExpectUTXO := func(name string, expected bool, op wire.OutPoint, hash chainhash.Hash) {
		tests = append(tests, []TestInstance{
	...
	for i := uint16(0); i < coinbaseMaturity; i++ {
		...
		g.saveTipCoinbaseOut()
		testInstances = append(testInstances, acceptBlock(g.tipName, g.tip, true, false))
	}
