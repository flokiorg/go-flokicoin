...
import (
	...
	"github.com/flokiorg/go-flokicoin/blockchain/fullblocktests"
...
func TestFullBlocks(t *testing.T) {
	...
	chain, teardownFunc, err := chainSetup("fullblocktest", &chaincfg.RegressionNetParams)
	if err != nil {
	...
	testAcceptedBlock := func(item fullblocktests.AcceptedBlock) {
		...
		block := chainutil.NewBlock(item.Block)
		block.SetHeight(blockHeight)
		t.Logf("Testing block %s (hash %s, height %d)", item.Name, block.Hash(), blockHeight)
	...
	testRejectedNonCanonicalBlock := func(item fullblocktests.RejectedNonCanonicalBlock) {
		...
		err := msgBlock.FlcDecode(bytes.NewReader(item.RawBlock), 0, wire.BaseEncoding)
