...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 FLC (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
			name:         "546 Loki (current network dust value)",
			uncompressed: 546,
			compressed:   4911,
		},
		{
			name:         "0.00001 FLC (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 FLC (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
		...
		{
			name:         "21000000 FLC (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "pay-to-pubkey uncompressed 1 FLC",
			amount:     100000000,
