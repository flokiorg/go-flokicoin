...
// BlockChain provides functions for working with the flokicoin block chain.
...
func (b *BlockChain) ChainWork(hash *chainhash.Hash) (*big.Int, error) {
	...
	if node == nil {
		return big.NewInt(0), fmt.Errorf("block %s is not known", hash)
	}
...
func (b *BlockChain) verifyReorganizationValidity(detachNodes, attachNodes *list.List) (
	[]*chainutil.Block, []*chainutil.Block, [][]SpentTxOut, error) {
...
func (b *BlockChain) isCurrent() bool {
	...
	// Not current if the latest best block has a timestamp before 40 days
	// ago.
	//
	// The chain appears to be current if none of the checks reported
	// otherwise.
	minus24Hours := b.timeSource.AdjustedTime().Add(-40 * 24 * time.Hour).Unix() // 40 days
